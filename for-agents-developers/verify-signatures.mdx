...
<Steps>
    <Step title="Get the LLM inference response as in quickstart">
        From the previous step the LLM inference result is returned with `hash`, `public_key` and `signature` that will be used to verify that the result came from the TEE.
