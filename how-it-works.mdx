...
For details on implementation, please see the [Solidity reference](/reference).
...
1. Trust that the TEE itself is correctly manufactured.
2. Verify that the enclave image is correct. To do so, you need to
    1. build the oracle docker image,
    2. convert the docker image to a Nitro Enclave image,
    3. verify that the Nitro Enclave image hash is correct (i.e. the hash Galadriel's TEE oracle uses is equal to the hash you received), proving that the enclave image is correctly assembled from the docker image.
3. Verify the attestation that a TEE-executed oracle is correct, and the enclave image hash included in the attestation matches the hash from step (2.3).
...
* An [interactive explanation](https://teeml.galadriel.com/) of the TEE setup and verification.
* Repository containing the enclave setup and instructions for verifying attestations: see [galadriel-ai/teeML](https://github.com/galadriel-ai/teeML.git).
...
* Implementations of the above on in [galadriel-ai/contracts](https://github.com/galadriel-ai/contracts):
    ...
    * [Oracle contract](https://github.com/galadriel-ai/contracts/blob/main/contracts/contracts/ChatOracle.sol)
* Full [Solidity reference](/reference)