...
* `int8 frequencyPenalty` - an integer between -20 and 20, mapped to a float between -2.0 and 2.0; values greater than 20 are treated as null.
...
* `string content` - the model output. **Either this field or `functionArguments` and `functionName` will be populated**, depending on whether the output is a normal message or a function call.
* `string functionArguments`. TODO-Kristjan what format if function?
