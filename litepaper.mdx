...
Trusted execution environments (TEEs) have also been recently [proposed to solve Ethereum's MEV](https://www.youtube.com/watch?v=Xq7oWtiwWII&t=2218s) by producing ETH blocks in TEEs, which show its promise in blockchain applications. teeML's strengths are low latency, cost, and high throughput. On the downside, it can't be verified on-chain, which means it's not a fully trustless system. We at Galadriel run the teeML oracle, which means it's akin to a centralized sequencer in the case of L2s. For a more trustless setup, developers can also fork or run their own teeML oracle if they choose to. We also aim to incentivize more oracles to provide on-chain inference. 

### Validator level inference

teeML is only the first step and as part of our roadmap, we are planning to develop a fully verifiable AI inference on the node level that can run any open-source model. This meets the needs of securing high TVL use cases as it is trustless and is part of the network consensus. We will be writing more about this in the future.
...
On-chain Midjourney: [https://minter.galadriel.com/](https://minter.galadriel.com/)
...
On-chain ChatGPT: [https://chatgpt.galadriel.com/](https://chatgpt.galadriel.com/)
...
Imagine you are part of a DAO but the managing of the treasury is in the hands of humans, which need trust and the DAO is also missing out on many investments because of lack of consensus. This is exact problem we've seen in our DAO and to solve it we are developing DAO Treasury AI.
...
Furthermore, we see the following use cases: payment network for agents, automated yield farming, agent based prediction markets, on-chain insurance & hedge funds managed by AI and all sorts of incentive designs for progressing AI.
...
Galadriel's Devnet is now out. During devnet developers can build working on-chain AI agents and applications. The focus is on developing the core technology and quickly iterating the product along with user needs. We expect to make many big architectural changes during the devnet as the full spec is yet to be implemented. 

### Testnet

After the validation of the design and architecture during devnet, we aim to lock the chain spec and release a testnet. It will act as the final testing ground before mainnet together with a security audit. We seek to reward and incentivise the early adopters and community members. 
...
# Join the mission in building a safe, citizen owned AI

Galadriel's long-term vision towards becoming ETH for AI is following:

1. Build the first L1 for AI;
2. Use that to onboard FLOPS to the network;
3. Use that to onboard user data to the network;
4. While doing above, become the world's biggest AI safety network where crypto-incentives drive humanity to align AI.

What is the future of AI? Will it be closed, corporate-owned, or safe and user-owned? Become a pirate and join the mission - provide feedback, questions, and stress test the existing approach. One thing is certain: the only way to predict the future is to invent it.