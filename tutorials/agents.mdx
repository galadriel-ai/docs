import {agentAddress} from '/snippets/addresses.mdx';
...
<Tip>
    Try out and see the agent in action by running the `agent.ts` [script](https://github.com/galadriel-ai/contracts/tree/main/examples) that calls already deployed contract.
...
<Steps>
    <Step name="Make a call to the agent">

    Execute the following command to run a [script](https://github.com/galadriel-ai/contracts/blob/main/examples/agent.ts) that calls the deployed contract:
