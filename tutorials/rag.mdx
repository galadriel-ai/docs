...
- An API key for `pinata.cloud` to facilitate document uploading to IPFS. You can obtain this key by registering at [pinata.cloud](https://pinata.cloud).
...
<Steps>
    ...
    <Step name="Create a .env file">
        Create a `.env` file and add your `pinata.cloud` API key and wallet private key as follows:
