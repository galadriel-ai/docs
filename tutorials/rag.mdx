...
Now we can execute the script that will extract the text from the documents in the `kb/` directory and upload them to IPFS. After uploading the documents, the script will request the oracle to index the documents. Once the index is complete, the script will output the IPFS link to the index.
...
Make sure you store the index IPFS link from this output as we will need it later -- in this example it is `TODO`.
