...
In this tutorial, we’ll build a simple LLM contract using Solidity on Galadriel (Devnet). We’ll use GPT-4-turbo without extra chat history or additional features, to demonstrate how teeML functions and how to call it.

To use a different LLM like `Claude-3.5-Sonnet`, `Mistral7B`, or [any other available with teeML](https://docs.galadriel.com/teeML-features) follow through with the tutorial and find code references in the end.
