...
```
<Info>
GitHub import is commented out as it's not working with local developemnt using Hardhat.
</Info>
### Variables
...
```solidity
string message; // example: tell me about cats
```

The oracle will store the response in the `response` variable.

```solidity
string response; // example: cats are...
```
...
Notice the `message` variable is not sent to the oracle with `createLlmCall`, but rather stored on the same contract. This is done by the helper function `createTextMessage`.
...
`createLlmCall` will call an oracle with the following parameters
...
After LLM returns the response to teeML, it gets pushed on-chain back to oracle, and from there the oracle calls `onOracleLlmResponse` callback function in your contract.

- The funciont must be called `onOracleLlmResponse`
- Include `unint` parameter to keep track of the user message to respond to (we hardcoded this to `0`)
- Includes parameter `IOracle.LlmResponse` with the correct response
...
- [Example](https://github.com/galadriel-ai/contracts/tree/main/examples) scripts to call out deployed contracts (currently missing for `SimpleLLM`).
- Use other LLMs: 
