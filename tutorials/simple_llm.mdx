...
In this tutorial, we’ll build a simple LLM contract using Solidity on the Galadriel Network (Devnet). In this example, we’ll use GPT with no chat history. To build a LLM with chat history visit [Calling a LLM](https://docs.galadriel.com/tutorials/llm).
...
The address of the teeML [oracle](https://docs.galadriel.com/oracle-address) on Galadriel. For more information on how it works, click [here](https://docs.galadriel.com/how-it-works).
...
Create a public function to start the LLM. In this example, our function prompt accepts a string \_message. An example message is “tell me about cats”.

Our prompt will call an oracle with the following parameters

- runId is used to keep track of the messages and prompts
- config set in the constructor

The prompt will send the values to the [oracle](https://docs.galadriel.com/how-it-works) to process the correct response.

Notice the message is not sent to the oracle. This is because the oracle will read getMessageHistory to retrieve the message history. This reading (instead of writing) will save on gas and storage.
...
Once the oracle reads the message history, the oracle will respond by calling onOracleOpenAiLlmResponse
...
- Includes parameter runId used to keep track of the prompts
...
The contract then set response to the error message or the response from the oracle.

### Response

After the oracle responds, anyone can access the value by reading the response variable to view the response.
...
- Implement a more [advanced OpenAI Chat GPT.](https://github.com/galadriel-ai/contracts/blob/0f9c3d09be158b6068e74d69c2ee0ad933db9282/contracts/contracts/OpenAiChatGpt.sol).
