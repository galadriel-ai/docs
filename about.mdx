...
Vitalik initially intended to build smart contracts on Bitcoin but this idea was rejected by the core developers. That turned out to be an exceptionally good outcome because Ethereum needed vertical integration to enable smart contracts. 

We initially wanted to build Galadriel as a Layer 2 but couldn't. We realized that the chain has to be customized and optimized across the whole stack to enable AI apps and agents on Web3 rails. For example, you couldn't build an on-chain ChatGPT or Midjourney today, if the goal is to support 100,000 users or a 10,000 agent swarm. That's because blockchains so far haven't been designed to run AI applications. Purpose-built tools enable purpose-built use cases, and AI is one of them.

# What is Galadriel's mission?

Our mission is to build **safe, citizen owned AI by building ETH for AI**. That means the following:

* It acts as the settlement layer for AI, driving real GDP growth.
* It provides direct ownership, the means of distributing wealth, and governance over AI.
* It is the largest applied safety network where crypto-incentives drive wisdom of the crowds to align AI.